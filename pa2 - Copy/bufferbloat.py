...
def get_timings(net, h1, h2):
    timings = []
    #for i in range(3):
    for i in range(1): #change to curling just once according to the handout 
...
def bufferbloat():
	...
	measurements.append(get_timings(net, h1, h2)) #changed get_timings to just 1 time
        #sleep(1)
	sleep(2) #changing it to once every 2 seconds instead of three times every 1 second
        now = time() 
    ...
    plt.plot(download_times, measurements)
    plt.xlabel("Time")
    plt.ylabel("Download time (seconds)")
    plt.title("Webpage Download Times")
    plt.savefig("download-" + str(max_q) + ".png")
